package com.marceltessarini.lojavirtual.rs.codigos;
...
public class CodigoAPIService {
	...
	public static List<Erro> filtar(List<Erro> erros, String codigoHttp) {
		List<Erro> filtrado = new ArrayList<>();
		
		for (Erro erro:erros) {
			String codigo = erro.getCodigoHttp();
			if (codigo.equals(codigoHttp)) {
				filtrado.add(erro);
			}
		}
